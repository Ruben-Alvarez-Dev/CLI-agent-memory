"""MCP session — long-lived subprocess for MCP-agent-memory JSON-RPC."""
...
from .discovery import discover_mcp_server_dir
from .mcp_env import load_mcp_env

logger = logging.getLogger(__name__)

# Resolved at import time (singleton)
_MCP_BASE = discover_mcp_server_dir()
_MCP_ENV = load_mcp_env(_MCP_BASE)
...
class MCPSessionManager:
    ...
    async def start(self) -> None:
        ...
        await self._send_json({"jsonrpc": "2.0", "method": "initialize",
            ...
            "clientInfo": {"name": "cli-agent-memory", "version": "0.3.0"}}, "id": self._next_id()})
